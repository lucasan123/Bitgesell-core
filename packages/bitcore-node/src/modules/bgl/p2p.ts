...
import { BitcoinBlock, BitcoinBlockStorage, iBglBlock } from '../../models/block';
...
import { BitcoinBlockType, BitcoinHeaderObj, BitcoinTransaction } from '../../types/namespaces/Bitgesell';
import { wait } from '../../utils/wait';

export class BitcoinP2PWorker extends BaseP2PWorker<iBglBlock> {
  ...
  setupListeners() {
    ...
    this.pool.on('peerconnect', peer => {
      logger.info(
        `${timestamp()} | Connected to connect-peer: ${peer.host}:${peer.port.toString().padEnd(5)} | Chain: ${
          this.chain
        } | Network: ${this.network}`
      );
    });
