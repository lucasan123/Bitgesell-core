...
import { BitcoinBlockStorage, iBglBlock } from '../../../models/block';
...
export class InternalStateProvider implements IChainStateService {
  ...
  async getBlocks(params: GetBlockParams): Promise<Array<IBlock>> {
    ...
    const blockTransform = (b: iBglBlock) => {
      let confirmations = 0;
      if (b.height && b.height >= 0) {
        confirmations = tipHeight - b.height + 1;
      }
      const convertedBlock = BitcoinBlockStorage._apiTransform(b, { object: true }) as iBglBlock;
  ...
  private extractAddress(address: string): string {
    const extractedAddress = address.replace(/^(bitgesellcash:|bchtest:|bitgesell:)/i, '').replace(/\?.*/, '');
