...
describe('Address Validation', () => {
  ...
  // Uri
  const btcUri = 'bitgesell:1NuKwkDtCymgA1FNLUBaUWLD8s4kdKWvgn';
  const bchUri = 'bitgesellcash:pp8skudq3x5hzw8ew7vzsw8tn4k8wxsqsv0lt0mf3g';
  ...
  const btcTestUri = 'bitgesell:mkUNMewkQsHKRcUvv5HLKbqmepCqNH8goc';
  ...
  it('should be able to validate an BGL address', async () => {
    const isValidAddress = await Validation.validateAddress('BGL', 'mainnet', btcAddress);
    const isValidTestAddress = await Validation.validateAddress('BGL', 'testnet', btcTestAddress);
  ...
  it('should be able to validate an BGL Uri', async () => {
    const isValidUri = await Validation.validateUri('BGL', btcUri);
    const isValidTestUri = await Validation.validateUri('BGL', btcTestUri);
  ...
  it('should be able to invalidate an incorrect BGL address', async () => {
    const inValidAddress = await Validation.validateAddress('BGL', 'mainnet', invalidBtcAddress);
