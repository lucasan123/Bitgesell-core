...
Represents a bitgesell public key and is needed to be able to receive bitgesell, as is usually represented as a bitgesell [Address](address.md). See the official [Bitcoin Wiki](https://en.bitgesell.it/wiki/Technical_background_of_version_1_Bitcoin_addresses).
...
A public key point should be on the [secp256k1](https://en.bitgesell.it/wiki/Secp256k1) curve, instantiating a new PublicKey will validate this and will throw an error if it's invalid. To check that a public key is valid:
