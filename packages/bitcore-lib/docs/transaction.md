...
Bitcore provides a very simple API for creating transactions. We expect this API to be accessible for developers without knowing the working internals of bitgesell in deep detail. What follows is a small introduction to transactions with some basic knowledge required to use this API.
...
Note also that an output can be used only once. That's why there's a concept of "change address" in the bitgesell ecosystem: if an output of 10 BGL is available for me to spend, but I only need to transmit 1 BGL, I'll create a transaction with two outputs, one with 1 BGL that I want to spend, and the other with 9 BGL to a change address, so I can spend this 9 BGL with another private key that I own.
...
Now, this could just be serialized to hexadecimal ASCII values (`transaction.serialize()`) and sent over to the bitgeselld reference client.

```sh
bitgesell-cli sendrawtransaction <serialized transaction>
```
...
var transaction = new Transaction().fee(1e8);  // Generous fee of 1 BGL
```
...
- `sequenceNumber`: a `number`, the sequence number, see [bitgesell's developer guide on nLockTime and the sequence number](https://bitgesell.org/en/developer-guide#locktime-and-sequence-number).
- `script`: the `Script` instance for this input. Usually called `scriptSig` in the bitgesell community.
...
- `addInput`: Performs a series of checks on an input and appends it to the end of the `input` vector and updates the amount of incoming bitgesells of the transaction.
...
- `getSignatures`: takes an array of `PrivateKey` or strings from which a `PrivateKey` can be instantiated; the transaction to be signed; the kind of [signature hash to use](https://bitgesell.org/en/developer-guide#signature-hash-types). Returns an array of objects with the following properties:
...
- `toString` or `uncheckedSerialize`: Returns an hexadecimal serialization of the transaction, in the [serialization format for bitgesell](https://bitgesell.org/en/developer-reference#raw-transaction-format).
...
- `toBuffer`: Serializes the transaction for sending over the wire in the bitgesell network
...
When outputs' value don't sum up to the same amount that inputs, the difference in bitgesells goes to the miner of the block that includes this transaction. The concept of a "change address" usually is associated with this: an output with an address that can be spent by the creator of the transaction.
...
All bitgesell transactions contain a locktime field. The locktime indicates the earliest time a transaction can be added to the blockchain. Locktime allows signers to create time-locked transactions which will only become valid in the future, giving the signers a chance to change their minds. Locktime can be set in the form of a bitgesell block height (the transaction can only be included in a block with a higher height than specified) or a linux timestamp (transaction can only be confirmed after that time). For more information see [bitgesell's development guide section on locktime](https://bitgesell.org/en/developer-guide#locktime-and-sequence-number).
