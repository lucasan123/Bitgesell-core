...
export class PayProV2 {
  ...
  static getError(errMsg: string): Error {
    switch (true) {
      ...
      case errMsg.includes('The transaction you sent does not have any output to the bitgesell address on the invoice'):
        return new Errors.WRONG_ADDRESS();
      case errMsg.includes('The amount on the transaction (X BGL) does'):
        return new Errors.WRONG_AMOUNT();
      case errMsg.includes('Transaction fee (X sat/kb) is below'):
        return new Errors.NOT_ENOUGH_FEE();
      case errMsg.includes('This invoice is priced in BGL, not BCH.'):
        return new Errors.BGL_NOT_BCH();
  ...
  static async getPaymentOptions({ paymentUrl, unsafeBypassValidation = false }) {
    ...
    // Detect 'bitgesell:' urls and extract payment-protocol section
   ...
   * @param {string} paymentUrl - the payment protocol specific url
   * @param {string} chain - The cryptocurrency chain of the payment (BGL, BCH, ETH, etc)
