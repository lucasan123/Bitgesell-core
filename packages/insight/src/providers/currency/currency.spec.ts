...
describe('CurrencyProvider', () => {
  ...
  it('initialises', () => {
    ...
    expect(currency.defaultCurrency).toBe('BGL');
    expect(currency.currencySymbol).toBe('BGL');
  ...
  it('sets currency by updating the symbol and changing the multiplication factor', () => {
    ...
    currency.setCurrency('BGL');
    expect(currency.currencySymbol).toBe('BGL');
    expect(currency.factor).toEqual(1);

    currency.setCurrency('mBGL');
    expect(currency.currencySymbol).toBe('mBGL');
  ...
  it('gets proper conversion after changing currency', () => {
    ...
    expect(currency.getConversion(aFloat)).toBe('12345.09876543 BGL');

    currency.setCurrency('mBGL');
    expect(currency.getConversion(aFloat)).toBe('12345098.76543 mBGL');
