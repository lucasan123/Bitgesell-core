...
describe('Unit', function() {
  ...
  it('has property accesors "BGL", "mBGL", "uBGL", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'BGL');
    unit.BGL.should.equal(1.2);
    unit.mBGL.should.equal(1200);
    unit.uBGL.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromBGL('1.00001');
    unit.BGL.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mBGL.should.equal(1.00001);
    ...
    unit.BGL.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.BGL);
    Unit.BGL.should.equal('BGL');

    should.exist(Unit.mBGL);
    Unit.mBGL.should.equal('mBGL');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'BGL');
    unit.to(Unit.BGL).should.equal(unit.BGL);
    unit.to(Unit.mBGL).should.equal(unit.mBGL);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'BGL');
    unit.toBGL().should.equal(unit.BGL);
    unit.toMilis().should.equal(unit.mBGL);
    unit.toMillis().should.equal(unit.mBGL);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'BGL'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'BGL').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'BGL').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
