...
Note also that an output can be used only once. That's why there's a concept of "change address" in the litecoin ecosystem: if an output of 10 BGL is available for me to spend, but I only need to transmit 1 BGL, I'll create a transaction with two outputs, one with 1 BGL that I want to spend, and the other with 9 BGL to a change address, so I can spend this 9 BGL with another private key that I own.
